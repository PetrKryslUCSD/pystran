...
# 
# The comparison is made for four modes of vibration. The mode shapes correlate
...
# ## Conclusions
# 
# The free vibration of a 3D frame is solved and compared with an analytical solution. The agreement is good but not perfect. However, that happens because  a single  element per member was used  in the reference, and our model is richer, having three elements per member. If we force the number of elements per member to be the same (a single element per member), the agreembent of the frequencies becomes perfect. This illustrates the simple fact that more than one element per member is required for decent accuracy of the natural frequencies. 
